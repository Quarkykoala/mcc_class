...
app.post('/api/letters', async (req, res) => {
    ...
    if (id) {
        ...
        let finalTagIds = [];
        if (tagIdsProvided) {
            finalTagIds = (0, letter_utils_1.normalizeTagIds)(tag_ids);
            const { error: deleteError } = await supabase.from('letter_tags').delete().eq('letter_id', id);
            if (deleteError)
                return res.status(500).json({ error: deleteError.message });
            if (finalTagIds.length > 0) {
                const tagsToInsert = finalTagIds.map((tagId) => ({ letter_id: id, tag_id: tagId }));
            ...
            const { data: existingTags, error: tagReadError } = await supabase
                ...
                .eq('letter_id', id);
            if (tagReadError)
                return res.status(500).json({ error: tagReadError.message });
        ...
        const finalDepartmentId = updateData.department_id || currentLetter.department_id;
        const contentHash = (0, letter_utils_1.buildContentHash)({
            letterId: id,
    ...
    // 2. Assign Tags
    const normalizedTagIds = (0, letter_utils_1.normalizeTagIds)(tag_ids);
    if (normalizedTagIds.length > 0) {
        const tagsToInsert = normalizedTagIds.map((tagId) => ({ letter_id: letter.id, tag_id: tagId }));
...
app.post('/api/letters/:id/approve', async (req, res) => {
    const { id } = req.params;
...
app.post('/api/letters/:id/committee-approve', async (req, res) => {
    const { id } = req.params;
...
app.post('/api/letters/:id/issue', async (req, res) => {
    const { id } = req.params;
    ...
    if (letter.status !== 'APPROVED')
        return res.status(400).json({ error: 'Only approved letters can be issued' });
    ...
    const pdfBase64 = await (0, letter_utils_1.generateIssuancePdf)({
        context: letter.context,
    ...
    const { data: issuance, error: iError } = await supabase
        ...
        .single();
    if (iError)
        return res.status(500).json({ error: iError.message });
...
app.get('/api/verify/:hash', async (req, res) => {
    ...
    const verificationResponse = (0, letter_utils_1.buildVerificationResponse)(version);
    await supabase.from('audit_logs').insert([{
            actor_id: null,
            action: 'VERIFY',
